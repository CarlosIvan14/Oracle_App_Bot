...
export default function NewTaskModal({ projectId, sprintId, onCreated }) {
  ...
  useEffect(() => {
    ...
    /* 1. Rol */
    fetch(`http://159.54.153.189/api/project-users/role-user/project-id/${projectId}/user-id/${user.idUser}`)
      .then(r => r.ok ? r.text() : Promise.reject())
      .then(txt => setRole(txt.trim()))
      .catch(()  => setRole(null));

    /* 2. mi idProjectUser (dev) */
    fetch(`http://159.54.153.189/api/project-users/project-id/${projectId}/user-id/${user.idUser}`)
      .then(r => r.ok ? r.text() : null)
      .then(txt => setMyProjectUserId(txt ? Number(txt) : null))
      .catch(() => setMyProjectUserId(null));

    /* 3. lista de usuarios del proyecto */
    fetch(`http://159.54.153.189/api/project-users/project/${projectId}/users`)
      .then(r => r.ok ? r.json() : [])
      .then(setAllUsers)
      .catch(() => setAllUsers([]));
  ...
  const fetchAiRecommendation = async () => {
    ...
    setAiLoading(true);
    try {
      const res = await fetch('http://159.54.153.189/assignment/by-ai', {
        method : 'POST',
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      const res = await fetch('http://159.54.153.189/api/tasks', {
        method :'POST',
        headers:{ 'Content-Type':'application/json' },
    ...
    if (needAssignee) {
      ...
      if (role==='developer') {
        ...
        try {
          const resPU = await fetch(
            `http://159.54.153.189/api/project-users/project-id/${projectId}/user-id/${selectedUserId}`
          );
          if (resPU.ok) {
      ...
      if (idProjectUser) {
        try {
          await fetch('http://159.54.153.189/api/task-assignees', {
            method :'POST',
            headers:{ 'Content-Type':'application/json' },
            body   : JSON.stringify({
