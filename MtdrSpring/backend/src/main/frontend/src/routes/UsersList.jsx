  ...
  function UsersList() {
          ...
          return Promise.all(
            usersData.map((user) =>
              fetch(`http://159.54.153.189/api/project-users/role-user/project-id/${projectId}/user-id/${user.idUser}`)
                ...
                .then((roleText) =>
                  fetch(`http://159.54.153.189/api/skills/oracleuser/${user.idUser}`)
                    .then((respSkill) => {
                      if (!respSkill.ok) {
    ...
    const saveEditSkill = async (skillId, newName, newDesc) => {
      ...
      try {
        const response = await fetch(`http://159.54.153.189/api/skills/${skillId}`, {
          method: "PATCH",
          headers: {
    ...
    const saveNewSkill = async (newName, newDesc) => {
      ...
      try {
        const response = await fetch(`http://159.54.153.189/api/skills`, {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
