...
function Reports() {
  ...
  useEffect(() => {
    fetch(`${config.apiBaseUrl}/api/projects/${projectId}`)
      ...
      .then(data => {
        ...
        setMembers([{ id: "all", name: "Todo el equipo", color: "#8884d8" }, ...devs]);
  ...
  const handleGenerateReport = async () => {
    if (!selectedMember) { alert("Selecciona un miembro."); return; }

    /* ---------- 1. KPI ORIGINALES (sin cambios) ---------------- */
    const isTeam = selectedMember === "all";
    let doneCountURL = "", doneDataURL = "";
    if (filterType === "sprint") {
      if (!selectedSprint) { alert("Selecciona un sprint."); return; }
      doneCountURL = isTeam
      ...
      if (!selectedDate) { alert("Selecciona fecha."); return; }
      doneCountURL = isTeam
    ...
    try {
      ...
      /* ------ métricas KPI ----- */
      let est=0, real=0, spTot=0, spFreq={};
      ...
      const spChart = Object.entries(spFreq).map(([sp,c])=>({
        name:`${sp} SP`, value:c,
        percent: ((c / (Object.values(spFreq).reduce((a,b)=>a+b,0) || 1))*100).toFixed(1)
      }));
      ...
      const g1 = await Promise.all(
        sprints.map(async sp => {
          const h = await fetch(
            `${config.apiBaseUrl}/api/timelogs/team-sprint/${sp.id}/real-hours`
          ).then(r=>r.json()).catch(()=>({hours:0}));
      ...
      const g2Promises = sprints.map(async sp => {
        ...
        await Promise.all(devs.map(async d => {
          const h = await fetch(
            `${config.apiBaseUrl}/api/timelogs/individual-sprint/${sp.id}/${d.id}/real-hours`
          ).then(r=>r.json()).catch(()=>({hours:0}));
          row[d.id] = h?.hours ?? 0;
        }));
        return row;
      ...
      const [g2, g3] = await Promise.all([Promise.all(g2Promises), Promise.all(g3Promises)]);
      setHoursByDevPerSprint(g2);
      setTasksByDevPerSprint(g3);

      /* --------------- guardar KPI / tabla --------------------- */
      setReportData({
        user: members.find(m=>String(m.id)===String(selectedMember))?.name ?? "—",
        tasksDone: cnt,
        realHours: real,
        estimatedHours: est,
        storyPoints: spTot,
        ...
        tasksData: processed.sort((a,b)=>(
          (a.projectUser?.user?.name ?? "").localeCompare(
            b.projectUser?.user?.name ?? "", "es", {sensitivity:"base"})
        )),
      });
  ...
  const barChartData = useMemo(
    () => reportData?.tasksData?.map(r=>({
      name:r.task.name, real:r.task.realHours, estimated:r.task.estimatedHours,
    })) ?? [], [reportData]
  );

  const PIE_COLORS = ["#ffffff","#d3d3d3","#a9a9a9","#808080",
                      "#585858","#2f2f2f","#1a1a1a","#000000"];
  ...
  return (
    <div className="p-8 space-y-10">
      ...
      <div className="flex flex-wrap gap-4 items-end">
        {/* filtro */}
        <div>
          <label className="block text-sm text-gray-200 font-bold mb-1">Filtrar por</label>
          <select value={filterType} onChange={e=>setFilterType(e.target.value)}
            className="bg-customDark bg-opacity-30 text-white px-3 py-2 rounded-xl">
            <option value="sprint">sprint</option><option value="week">semana</option><option value="month">mes</option>
          </select>
        </div>

        {filterType==="sprint" && (
          <div>
            <label className="block text-sm text-gray-200 font-bold mb-1">Sprint</label>
        ...
        {filterType!=="sprint" && (
          <div>
            ...
            <DatePicker selected={selectedDate?new Date(selectedDate):null}
              onChange={d=>setSelectedDate(d?d.toISOString().slice(0,10):"")}
              className="bg-customDark bg-opacity-30 text-white px-3 py-2 rounded-xl"
              placeholderText="Selecciona fecha" dateFormat="yyyy-MM-dd"/>
          </div>
        )}

        {/* miembro */}
        <div>
          <label className="block text-sm text-gray-200 font-bold mb-1">Miembro</label>
        ...
        <div>
          ...
          <ResponsiveContainer width="100%" height={260}>
            <BarChart data={totalHoursBySprint}>
              ...
              <XAxis dataKey="sprintName" stroke="#ccc"/><YAxis stroke="#ccc"/>
              <Tooltip/><Bar dataKey="hours" fill="#c084fc" name="Horas"/>
            </BarChart>
          </ResponsiveContainer>
        </div>
      )}
        ...
        <div>
          ...
          <ResponsiveContainer width="100%" height={300}>
            <BarChart data={hoursByDevPerSprint}>
              ...
              <XAxis dataKey="sprintName" stroke="#ccc"/><YAxis stroke="#ccc"/>
              <Tooltip/><Legend/>
              {members.filter(m=>m.id!=="all").map(d=>(
                <Bar key={d.id} dataKey={String(d.id)} name={d.name} fill={memberColors[d.id]}/>
              ))}
            </BarChart>
          </ResponsiveContainer>
        </div>
      ...
      {barChartData.length>0 && (
        <div className="grid grid-cols-6 gap-6 mt-10">
          <div className="col-span-1 bg-black/30 rounded-2xl p-6 shadow-lg">
            ...
            <p className="text-3xl font-bold text-blue-400">{reportData?.kpi1}</p>
          </div>
      ...
      {reportData?.spFrequencyChartData?.length>0 && (
        <div className="grid grid-cols-6 gap-6 mt-10">
          <div className="col-span-1 bg-black/30 rounded-2xl p-6 shadow-lg">
            ...
            <p className="text-3xl font-bold text-blue-400">{reportData.kpi2}</p>
          </div>
          <div className="col-span-5 bg-black/30 rounded-2xl flex justify-center p-4">
            <PieChart width={400} height={400}>
              <Pie data={reportData.spFrequencyChartData} cx="50%" cy="50%"
              ...
              <Tooltip formatter={(v,n,p)=>[`${v} tasks`,p.payload.name]}/>
              <Legend/>
            </PieChart>
          </div>
        </div>
            ...
            <table className="min-w-full text-sm bg-black/50 text-white">
              ...
              <tbody className="divide-y divide-gray-700">
                {reportData.tasksData.map(r=>{
                  ...
                  const rh = t.realHours||0, eh = t.estimatedHours||0;
