...
import { 
  BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, Legend, ResponsiveContainer, // BarChart KPI 1
  PieChart, Pie, Cell // Pie Chart KPI 2
} from 'recharts';
...
function Reports() {
  ...
  useEffect(() => {
    fetch(`http://159.54.153.189/api/projects/${projectId}`)
      .then(res => res.json())
      .then(data => {
  ...
  const handleGenerateReport = async () => {
    ...
    if (filterType === 'sprint') {
      ...
      doneTasksCountEndpoint = isTeam
        ? `http://159.54.153.189/api/task-assignees/team-sprint/${selectedSprint}/done/count`
        : `http://159.54.153.189/api/task-assignees/user/${selectedMember}/sprint/${selectedSprint}/done/count`;

      doneTasksDataEndpoint = isTeam
        ? `http://159.54.153.189/api/task-assignees/team-sprint/${selectedSprint}/done`
        : `http://159.54.153.189/api/task-assignees/user/${selectedMember}/sprint/${selectedSprint}/done`;

      // realHoursEndpoint = isTeam
      //   ? `http://159.54.153.189/api/timelogs/team-sprint/${selectedSprint}/real-hours`
      //   : `http://159.54.153.189/api/timelogs/individual-sprint/${selectedSprint}/${selectedMember}/real-hours`;

      // timeLogsEndpoint = isTeam
      //    ? `http://159.54.153.189/api/timelogs/team-sprint/${selectedSprint}/time-logs`
      //    : `http://159.54.153.189/api/timelogs/individual-sprint/${selectedSprint}/${selectedMember}/time-logs`;
      ...
      doneTasksCountEndpoint = isTeam
        ? `http://159.54.153.189/api/task-assignees/team-week/${selectedDate}/project/${projectId}/done/count`
        : `http://159.54.153.189/api/task-assignees/user/${selectedMember}/week/${selectedDate}/done/count`;

      doneTasksDataEndpoint = isTeam
        ? `http://159.54.153.189/api/task-assignees/team-week/${selectedDate}/project/${projectId}/done`
        : `http://159.54.153.189/api/task-assignees/user/${selectedMember}/week/${selectedDate}/done`;

      // realHoursEndpoint = isTeam
      //   ? `http://159.54.153.189/api/timelogs/team-week/${selectedDate}/${projectId}/real-hours`
      //   : `http://159.54.153.189/api/timelogs/individual-week/${selectedDate}/${selectedMember}/real-hours`;

      // timeLogsEndpoint = isTeam
      //   ? `http://159.54.153.189/api/timelogs/team-week/${selectedDate}/${projectId}/time-logs`
      //   : `http://159.54.153.189/api/timelogs/individual-week/${selectedDate}/${selectedMember}/time-logs`;
      ...
      doneTasksCountEndpoint = isTeam
        ? `http://159.54.153.189/api/task-assignees/team-month/${selectedDate}/project/${projectId}/done/count`
        : `http://159.54.153.189/api/task-assignees/user/${selectedMember}/month/${selectedDate}/done/count`;

      doneTasksDataEndpoint = isTeam
        ? `http://159.54.153.189/api/task-assignees/team-month/${selectedDate}/project/${projectId}/done`
        : `http://159.54.153.189/api/task-assignees/user/${selectedMember}/month/${selectedDate}/done`;

      // realHoursEndpoint = isTeam
      //   ? `http://159.54.153.189/api/timelogs/team-month/${selectedDate}/${projectId}/real-hours`
      //   : `http://159.54.153.189/api/timelogs/individual-month/${selectedDate}/${selectedMember}/real-hours`;

      // timeLogsEndpoint = isTeam
      //   ? `http://159.54.153.189/api/timelogs/team-month/${selectedDate}/${projectId}/time-logs`
      //   : `http://159.54.153.189/api/timelogs/individual-month/${selectedDate}/${selectedMember}/time-logs`;
    ...
    try {
      ...
      const totalSp = Object.values(storyPointsFrequency).reduce((a, b) => a + b, 0);

      const spFrequencyChartData = Object.entries(storyPointsFrequency).map(([sp, count]) => ({
        name: `${sp} SP`,
        value: count,
      ...
      <div className="mt-10 grid grid-cols-6 gap-6">
        ...
        {barChartData && barChartData.length > 0 && (
          <div className="col-span-5 bg-black bg-opacity-30 p-4 rounded-2xl">
            <h2 className="text-xl font-semibold mb-4 text-white">Comparación de Horas por Tarea</h2>
            <ResponsiveContainer width="100%" height={300}>
              ...
              >
                ...
                <Tooltip />
                <Legend />
                <Bar dataKey="estimated" fill="#d3d3d3" name="Horas Estimadas" />
        ...
        {reportData?.spFrequencyChartData?.length > 0 && (
          <div className="col-span-5 bg-black bg-opacity-30 p-4 rounded-2xl flex justify-center items-center">
            <div className="flex justify-center items-center">
              <h2 className="text-xl font-semibold mb-4 text-white">Dificultad de las tareas</h2>
