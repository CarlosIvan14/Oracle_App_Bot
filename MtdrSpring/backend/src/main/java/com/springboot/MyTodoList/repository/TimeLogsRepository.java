...
public interface TimeLogsRepository extends JpaRepository<TimeLogs, Integer> {
	...
	@Query("SELECT tl FROM TimeLogs tl " + "WHERE tl.taskAssignees.projectUser.id = ?1 "
			+ "AND tl.taskAssignees.task.sprint.id = ?2 " + "AND tl.taskAssignees.task.status = 'COMPLETED'")
	List<TimeLogs> findTimeLogsByProjectUserAndSprint(int projectUserId, int sprintId);

	// R08 y R09: User-From-To reports using completion date between start and end
	@Query("SELECT tl FROM TimeLogs tl " + "WHERE tl.taskAssignees.projectUser.id = ?1 "
			+ "AND tl.taskAssignees.task.status = 'COMPLETED' " + "AND tl.taskAssignees.task.deadline >= ?2 "
			+ "AND tl.taskAssignees.task.deadline < ?3")
	List<TimeLogs> findTimeLogsByProjectUserAndDateRange(int projectUserId, LocalDateTime from, LocalDateTime to);

	// R10: Team-Sprint reports
	@Query("SELECT tl FROM TimeLogs tl " + "WHERE tl.taskAssignees.task.sprint.id = ?1 "
	...
	@Query("SELECT tl FROM TimeLogs tl " + "WHERE tl.taskAssignees.task.sprint.project.id = ?1 "
			+ "AND tl.taskAssignees.task.status = 'COMPLETED' " + "AND tl.taskAssignees.task.deadline >= ?2 "
			+ "AND tl.taskAssignees.task.deadline < ?3")
	List<TimeLogs> findTimeLogsByTeamAndDateRange(int projectId, LocalDateTime from, LocalDateTime to);
