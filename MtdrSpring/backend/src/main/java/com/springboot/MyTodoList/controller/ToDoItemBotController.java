...
public class ToDoItemBotController extends TelegramLongPollingBot {
	...
	private enum Flow {
		...
		NONE, LOGIN, ADD_SPRINT, ADD_TASK, ADD_USER, TASK_COMPLETE, REPORTS
	...
	/* ===================== CONSTRUCTOR ======================== */
	public ToDoItemBotController(String botToken, String botName, String backendBaseUrl, ProjectsServiceBot projectsSvc,
			SprintsServiceBot sprintsSvc, TaskServiceBot taskSvc, TaskCreationServiceBot taskCreationSvc,
	...
	public void onUpdateReceived(Update update) {
		...
		ChatState st = chats.containsKey(chatId) ? chats.get(chatId) : createState(chatId);
		...
		if ("/menu".equalsIgnoreCase(txt)) {
			if (st.loggedUser != null)
				...
				send(chatId, "No hay sesión. /start", false);
			return;
		}

		/* ---------- navegación simple ---------- */
		if (navigate(chatId, txt, st))
			return;

		/* ---------- project / sprint ---------- */
		if (selectProject(chatId, txt, st))
			return;
		if (selectSprint(chatId, txt, st))
			return;

		/* ---------- botones de sprint ---------- */
		if ("📋 Todas las Tareas".equals(txt)) {
			showAllTasks(chatId, st.currentSprintId);
			return;
		}
		if ("➕ Add Task".equals(txt)) {
			startAddTask(chatId, st);
			return;
		}
		if (handleTaskButtons(chatId, txt, st))
			return;
		if (txt.startsWith("👤 ASSIGN-")) {
			handleAssign(chatId, txt, st);
			return;
		}
		...
		if (txt.startsWith("👥 Ver Usuarios Proyecto")) {
			...
			showUsers(chatId, pid);
			return;
		}
		if ("➕ Agregar Usuario".equals(txt)) {
			st.flow = Flow.ADD_USER;
			st.step = 1;
			listOracleUsers(chatId, st);
			return;
		}
		if ("➕ Añadir Sprint".equals(txt)) {
			st.flow = Flow.ADD_SPRINT;
			st.step = 1;
			send(chatId, "Nombre del nuevo sprint:", false);
			return;
		}
		if (txt.startsWith("Deshabilitar-")) {
			toggleSprint(chatId, Integer.parseInt(txt.substring(13)), "idle", st);
			return;
		}
		if (txt.startsWith("Habilitar-")) {
			toggleSprint(chatId, Integer.parseInt(txt.substring(10)), "Active", st);
			return;
		}
		...
		if (st.flow != Flow.NONE) {
			processFlow(chatId, txt, st);
			return;
		}
	}
	...
	private boolean navigate(long chatId, String txt, ChatState st) {
		...
		if ("⬅️ Volver a Proyectos".equals(txt) || "⬅️ Regresar a Proyectos".equals(txt)) {
		...
		if ("⬅️ Volver a Sprints".equals(txt)) {
			boolean isManager = roleSvc.isManagerInProject(st.currentProjectId, st.loggedUser.getIdUser());
	...
	private void processFlow(long chatId, String txt, ChatState st) {
	...
	private void loginFlow(long chatId, String txt, ChatState st) {
		...
		if (st.step == 2) {
			...
			if (u == null) {
				send(chatId, "Login fallido.", false);
				reset(st);
				return;
			}
			...
			rest.exchange(baseUrl + "/users/" + u.getIdUser(), HttpMethod.PATCH,
					new HttpEntity<HashMap<String, Object>>(p), OracleUser.class);
	...
	private void sprintFlow(long c, String txt, ChatState st) {
		if (st.step == 1) {
			st.newSprintName = txt;
			st.step = 2;
			send(c, "/confirmar para guardar ó /cancel", false);
			return;
		}
	...
	private void createTask(long chatId, ChatState st) {
		try {
			Tasks t = new Tasks();
			t.setName(st.tName);
			t.setDescription(st.tDesc);
			t.setDeadline(st.tDeadline.atStartOfDay());
			t.setStoryPoints(st.tSP);
			t.setEstimatedHours(st.tEst);
			t.setRealHours(0.0);
	...
	private void completeFlow(long c, String txt, ChatState st) {
		try {
			...
			Integer pu = taskCreationSvc.getProjectUserId(st.currentProjectId, st.loggedUser.getIdUser());
			listTasksForSprint(c, st.currentSprintId, pu);
			reset(st);
		}
		catch (Exception e) {
			send(c, "Número?", false);
	...
	private boolean handleTaskButtons(long chatId, String txt, ChatState st) {
		if (txt.startsWith("▶ START-")) {
			changeStatus(chatId, txt, "IN_PROGRESS", st);
			return true;
		}
		if (txt.startsWith("❌ CANCEL-")) {
			changeStatus(chatId, txt, "ASSIGNED", st);
			return true;
		}
		...
		if (txt.startsWith("✅ DONE-")) {
			st.flow = Flow.TASK_COMPLETE;
			st.step = 1;
			st.currentTaskId = id(txt);
			send(chatId, "Horas reales?", false);
			return true;
		}
		return false;
	}
	...
	/**
	 * Cierra la sesión del usuario en este chat, borra el estado de la conversación y
	 * quita el teclado permanente.
	 */
	...
	private void handleAssign(long chatId, String txt, ChatState st) {
		...
		Integer pu = taskCreationSvc.getProjectUserId(st.currentProjectId, st.loggedUser.getIdUser());
		if (pu == null) {
			send(chatId, "No se pudo asignar.", false);
			return;
		}
	...
	private void showSprintsForProject(long chatId, int projectId, boolean mgr) {
		...
		for (Sprint s : sprints) {
			if (!mgr && !"Active".equalsIgnoreCase(s.getDescription()))
				continue;
			String icon = "Active".equalsIgnoreCase(s.getDescription()) ? "🟢" : "🔴";
			KeyboardRow r = new KeyboardRow();
			r.add(icon + " " + s.getName() + " (ID: " + s.getId() + ") #SPRINT#");
			if (mgr) {
				String toggle = ("Active".equalsIgnoreCase(s.getDescription()) ? "Deshabilitar-" : "Habilitar-")
						+ s.getId();
				r.add(toggle);
			}
			rows.add(r);
	...
	private void listTasksForSprint(long chatId, int sprintId, int projectUserId) {
		...
		if (!assigned.isEmpty()) {
			...
			for (Tasks t : assigned)
				rows.add(row(t.getName() + " [ID: " + t.getId() + "]", "▶ START-" + t.getId()));
		}
		...
		if (!done.isEmpty()) {
			...
			for (Tasks t : done)
				rows.add(row(t.getName() + " [ID: " + t.getId() + "]", "↩ UNDO-" + t.getId()));
		}
	...
	private List<Tasks> filterByStatus(List<Tasks> list, String st) {
		...
		for (Tasks t : list)
			if (st.equalsIgnoreCase(t.getStatus()))
				out.add(t);
		return out;
	}
	...
	private void addUserFlow(long chatId, String txt, ChatState st) {
		...
		showUsers(chatId, st.currentProjectId);
		reset(st);
	}

	private void toggleSprint(long chatId, int sprintId, String newStatus, ChatState st) {
		HashMap<String, Object> p = new HashMap<String, Object>();
		p.put("description", newStatus);
		rest.exchange(baseUrl + "/api/sprints/" + sprintId, HttpMethod.PATCH,
				new HttpEntity<HashMap<String, Object>>(p), Sprint.class);
		boolean mgr = roleSvc.isManagerInProject(st.currentProjectId, st.loggedUser.getIdUser());
		showSprintsForProject(chatId, st.currentProjectId, mgr);
	}

	private void showUsers(long chatId, int projectId) {
		OracleUser[] arr = rest.getForObject(baseUrl + "/api/project-users/project/" + projectId + "/users",
				OracleUser[].class);
		StringBuilder sb = new StringBuilder("*Usuarios del proyecto:*\n\n");
		if (arr != null && arr.length > 0)
			for (OracleUser u : arr)
				sb.append("• ").append(u.getName()).append("\n");
		else
			sb.append("Sin usuarios.");

		ReplyKeyboardMarkup kb = new ReplyKeyboardMarkup();
		kb.setResizeKeyboard(true);
		List<KeyboardRow> rows = Arrays.asList(row("➕ Agregar Usuario"), row("⬅️ Volver a Sprints"));
		kb.setKeyboard(rows);
	...
	private List<OracleUser> getProjectUsers(int pid) {
		OracleUser[] arr = rest.getForObject(baseUrl + "/api/project-users/project/" + pid + "/users",
				OracleUser[].class);
	...
	/* helpers UI */
	private void send(long chat, String txt, boolean md) {
		send(chat, txt, null, md);
	}

	private void send(long chat, String txt, ReplyKeyboardMarkup kb) {
		send(chat, txt, kb, false);
	}
	...
	private KeyboardRow titleRow(String t) {
		return row(t);
	}

	private ChatState createState(long chatId) {
		ChatState s = new ChatState();
		chats.put(chatId, s);
		return s;
	}

	private void reset(ChatState st) {
		st.flow = Flow.NONE;
		st.step = 0;
	}

	private int id(String t) {
