...
public class TaskCreationServiceBot {
	...
	private final String apiBaseUrl = "http://140.84.174.78/api";
	...
	@Autowired
	public TaskCreationServiceBot(RestTemplateBuilder restTemplateBuilder, UserRoleServiceBot userRoleService) {
	...
	public TaskAssignees assignTask(int taskId, int projectUserId) {
		...
		ResponseEntity<TaskAssignees> response = restTemplate.postForEntity(url, request, TaskAssignees.class);
		return response.getBody();
	}
