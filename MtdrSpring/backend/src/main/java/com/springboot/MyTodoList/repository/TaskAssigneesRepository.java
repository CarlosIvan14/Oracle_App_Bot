...
public interface TaskAssigneesRepository extends JpaRepository<TaskAssignees, Integer> {
	...
	@Query("SELECT COUNT(ta) FROM TaskAssignees ta " + "WHERE ta.projectUser.idProjectUser = ?1 "
			+ "AND ta.task.status = 'COMPLETED' " + "AND ta.task.deadline >= ?2 " + "AND ta.task.deadline < ?3")
	long countDoneTasksByProjectUserAndDateRange(int projectUserId, LocalDateTime from, LocalDateTime to);

	@Query("SELECT ta FROM TaskAssignees ta " + "WHERE ta.projectUser.idProjectUser = ?1 "
			+ "AND ta.task.status = 'COMPLETED' " + "AND ta.task.deadline >= ?2 " + "AND ta.task.deadline < ?3")
	List<TaskAssignees> findCompletedTasksByProjectUserAndDateRange(int projectUserId, LocalDateTime from,
			LocalDateTime to);

	// R04: Team-Sprint reports
	@Query("SELECT COUNT(ta) FROM TaskAssignees ta " + "WHERE ta.task.sprint.id = ?1 "
			+ "AND ta.task.status = 'COMPLETED'")
	long countDoneTasksByTeamAndSprint(int sprintId);

	@Query("SELECT ta FROM TaskAssignees ta " + "WHERE ta.task.sprint.id = ?1 " + "AND ta.task.status = 'COMPLETED'")
	List<TaskAssignees> findCompletedTasksByTeamAndSprint(int sprintId);

	// R05 y R06: Team-Week reports
	@Query("SELECT COUNT(ta) FROM TaskAssignees ta " + "WHERE ta.task.status = 'COMPLETED' "
			+ "AND ta.task.sprint.project.idProject = ?1 " + "AND ta.task.deadline >= ?2 AND ta.task.deadline < ?3")
	long countDoneTasksByTeamAndDateRange(int projectId, LocalDateTime from, LocalDateTime to);

	@Query("SELECT ta FROM TaskAssignees ta " + "WHERE ta.task.status = 'COMPLETED' "
			+ "AND ta.task.sprint.project.idProject = ?1 " + "AND ta.task.deadline >= ?2 AND ta.task.deadline < ?3")
	List<TaskAssignees> findCompletedTasksByTeamAndDateRange(int projectId, LocalDateTime from, LocalDateTime to);
